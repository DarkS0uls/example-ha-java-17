...
public class UsersAdapter implements UsersPort {
    ...
    @Override
    public Mono<UsersMassiveDto> getAllUsers(Integer pageNumber, Integer pageSize, String status, String userName, String cellphone, Date createdDt) {
        log.info("UsersAdapter.getAllUsers, with page number: {}, and page size: {}", pageNumber, pageSize);
        return Mono.just(usersAdapterSpecificationRepository.findAll(buildUserSpecification(userName, status, cellphone, createdDt), PageRequest.of(pageNumber, pageSize - 1)))
    ...
    private UsersAdapterSpecification buildUserSpecification(String status, String userName, String cellphone, Date createdDt) {
        log.info("UsersAdapter.buildUserSpecification, with status: {}, userName: {}, cellphone: {}, createdDt: {}", status, userName, cellphone, createdDt.toString());
